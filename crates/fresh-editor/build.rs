...
/// Prepare every bundled plugin at compile time.
///
/// Bundling, transpiling and emitting declarations for the shipped plugins is
/// a pure function of their sources, which are fixed here. Running it once
/// during the build means the runtime does a table lookup instead of calling
/// into oxc -- on every editor start, and in every test process. Plugins the
/// user installs are not in the table and still take the runtime path.
///
/// The table is keyed on the source's length and fingerprint as well as its
/// name, so a user plugin sharing a file name with a bundled one cannot be
/// served the bundled plugin's code.
fn generate_prepared_plugins() {
    let plugins_dir = Path::new("plugins");
    println!("cargo:rerun-if-changed=plugins");

    let mut entries: Vec<(String, String)> = Vec::new();
    let read = match fs::read_dir(plugins_dir) {
        Ok(read) => read,
        Err(e) => {
            println!("cargo:warning=plugins dir unreadable ({e}); shipping an empty table");
            write_prepared_plugins(&[]);
            return;
        }
    };
    ...
    // not churn the rebuild for no reason.
    paths.sort();
    ...
    for path in paths {
        ...
        // A plugin that fails to prepare is left out rather than failing the
        // build: the runtime will try it live and report the error the way it
        // always has.
        let prepared = match fresh_parser_js::prepare_source(&path, &source) {
            Ok(p) => p,
            Err(e) => {
                println!("cargo:warning=plugin {file_name} not precompiled: {e}");
                continue;
            }
        };
        let declarations = match prepared.declarations {
        ...
        entry.push_str(&format!("        source_len: {},\n", source.len()));
        ...
        entry.push_str("    },");
        entries.push((file_name.clone(), entry));
    }

    let bodies: Vec<String> = entries.into_iter().map(|(_, body)| body).collect();
    write_prepared_plugins(&bodies);
}

fn write_prepared_plugins(bodies: &[String]) {
    let out_dir = std::env::var("OUT_DIR").expect("OUT_DIR");
    let file = format!(
        "// @generated by build.rs -- do not edit.\n\
         pub static PREPARED_PLUGINS: &[fresh_plugin_runtime::PreparedPluginEntry] = &[\n{}\n];\n",
        bodies.join("\n")
    );
